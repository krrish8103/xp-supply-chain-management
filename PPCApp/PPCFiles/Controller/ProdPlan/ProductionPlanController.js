...
app.controller('ProductionPlanController', ['$rootScope', '$scope', '$modal', 'ProductionPlanFactory', '$location', 'userModel', 'tempModel', 'messageModel', '$base64',
    function ($rootScope, $scope, $modal, ProductionPlanFactory, $location, userModel, tempModel, messageModel, $base64) {
        function initialize() {
            $scope.productionPlan = {};
            $scope.allProductionPlan = {};
            $scope.isError = false;
        ...
        function getAllProductionPlan() {                   //get data from session
            ProductionPlanFactory.getAllProductionPlan()
            .success(function (data) {
                momentDate = moment(data.datCreatedDate, "DD-MM-YYYY HH:mm");
                $scope.productionPlan.datCreatedDate = momentDate;
                $scope.allProductionPlan = data;
                $scope.dispProductionPlan = [].concat($scope.allProductionPlan);
            })
        ...
        $scope.AddProductionPlanClick = function () {
            resetInfoPanel();
            userModel.editRow = {};
            userModel.editMode = false;
            $scope.productionPlan = {};
            $location.path('/ProductionPlanAddEdit').replace();
        };
        $scope.CancelProductionPlanClick = function () {
            $location.path('/home').replace();
        };
        $scope.editProductionPlanClick = function (ProductionPlan) {
            resetInfoPanel();
            userModel.editMode = true;
            userModel.editRow = ProductionPlan;
            $location.path('/ProductionPlanAddEdit').replace();
        };
