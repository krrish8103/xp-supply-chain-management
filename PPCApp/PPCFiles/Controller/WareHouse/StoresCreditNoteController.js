...
app.controller('StoresCreditController', ['$rootScope', '$scope', '$modal', 'StoresCreditNoteFactory', '$location', 'userModel', 'tempModel', 'messageModel', '$base64',
    function ($rootScope, $scope, $modal, StoresCreditNoteFactory, $location, userModel, tempModel, messageModel, $base64) {
        function initialize() {
            $scope.storescreditNote = {};
            $scope.allstorescreditNote = {};
            $scope.isError = false;
        ...
        getAllStoreCreditNotes();
        function getAllStoreCreditNotes() {                   //get data from session
            StoresCreditNoteFactory.getAllStoreCreditNotes()
            .success(function (data) {
                momentDate = moment(data.datCreatedDate, "DD-MM-YYYY HH:mm");
                $scope.storescreditNote.datCreatedDate = momentDate;
                $scope.allstorescreditNote = data;
                $scope.dispstorescreditNote = [].concat($scope.allstorescreditNote);
            })
        ...
        $scope.AddStoresCreditNoteClick = function () {
            resetInfoPanel();
            userModel.editRow = {};
            userModel.editMode = false;
            $scope.storescreditNote = {};
            $location.path('/StoresCreditNoteAddEdit').replace();
        };
        $scope.CancelStoresCreditNoteClick = function () {
            $location.path('/home').replace();
        };
        $scope.editStoresCreditNoteClick = function (StoresCreditNote) {
            resetInfoPanel();
            userModel.editMode = true;
            userModel.editRow = StoresCreditNote;
            $location.path('/StoresCreditNoteAddEdit').replace();
        };

        $scope.setDocStatusToWorkingDraft = function (StoresCreditNote) {
            bootbox.confirm({
                ...
                callback: function (result) {
                    if (result) {
                        StoresCreditNote.lngDocStatus = 1;
                        StoresCreditNoteFactory.setStoresCreditNoteStatus(StoresCreditNote.lngId, StoresCreditNote)
                      .success(function (data) {
                          getAllWorkOrder();
